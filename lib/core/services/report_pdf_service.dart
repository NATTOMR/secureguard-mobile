...
class ReportPdfService {
  ...
  /// Computes a deterministic SHA-256 cryptographic audit digest for report non-repudiation
  static String computeAuditDigest({
    ...
    buffer.write('AUDIT_ID:$auditId|');
    buffer.write('TIMESTAMP:$timestamp|');
    buffer.write('FRAMEWORK:$framework|');
    buffer.write('SCORE:$postureScore|');
    if (controls != null) {
      for (final c in controls) {
        buffer.write('${c['id']}:${c['status']};');
      }
    }
  ...
  static Future<Uint8List> buildSecurityReportPdf({
    ...
    // Compute real cryptographic SHA-256 digest of report contents
    final auditDigest = computeAuditDigest(
      auditId: auditId,
      framework: framework,
            ...
            children: [
              pw.Column(
                ...
                children: [
                  ...
                  pw.Text('SHA256: $auditDigest', style: pw.TextStyle(fontSize: 7.5, color: PdfColor.fromHex('0F172A'), fontWeight: pw.FontWeight.bold)),
                  pw.SizedBox(height: 4),
